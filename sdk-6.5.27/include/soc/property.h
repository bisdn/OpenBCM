...
/*
 * Enable Dual OAMP Mode.
 * 0 - Single OAMP.
 * 1 - Dual OAMP - Two Independent OAMPs.
 */
#define spn_OAMP_DUAL_MODE  "oamp_dual_mode"
/*
 * Size of the host memory allocated by the CPU.
 * Value 0 indicates that the size will be determined by the threshold
 */
 ...
 * Valid values:
 * 0 = allocate from low portion, hard partition for overlay/underlay memory spaces(default, bit0 = 0, bit1 = 0)
 * 1, 3 = allocate from high portion (bit0 = 1)
 * 2, 3 = soft partition, adjust overlay/underlay partition on demand(bit1 = 1)
 */
 ...
 * NOTES:
 *    1. It is a WAR for H/W limitation.
...
/* Property to select overlay qualifiers for source and destination IPv4 and v6 qualifiers during group create. This enables the qualifiers (srcIp6, srcIp) or (dstIp6, dstIp) be part of same field selectors that helps in reducing the size of the TCAM width and might reduce the group mode from double-wide to single-wide. */
#define spn_FP_QUAL_IPV6_IPV4_OVERLAY_ENABLE  "fp_qual_ipv6_ipv4_overlay_enable"
...
/*
 * Disable using single wide key for IPMC in DEFIP table.
 * 0    --   Enable using single wide key for IPMC in DEFIP table (default).
 * 1    --   Disable using single wide key for IPMC in DEFIP table.
 */
#define spn_DEFIP_WIDE_KEY_FOR_IPMC_DISABLE  "defip_wide_key_for_ipmc_disable"
/*
 * 1 - enable MoD periodic counter exports through IPFIX format
 * default value: 0 (disabled)
 */
#define spn_FLOWTRACKER_EXPORT_IPFIX_ENABLE  "flowtracker_export_ipfix_enable"
/*
 * Specify if tunnel related extended fields such as src_port, dscp and ttl
 * can be updated per tunnel, in vxlan tunnel initiator create
 * 0    --   Disable updating the extended fields per tunnel.
 * 1    --   Enable updating the extended fields per tunnel.
 */
#define spn_VXLAN_TUNNEL_INIT_EXT_FIELDS_UPDATE  "vxlan_tunnel_init_ext_fields_update"
/*
 * Indicate reserved RDBGC counters.
 * reserved_rdbgc_x controls RDBGCx.
 * By default RDBGC1-2 are reserved.
 * 0    --   Unreserve the RDBGC counter.
 * 1    --   Reserve the RDBGC counter.
 */
#define spn_RESERVED_RDBGC  "reserved_rdbgc"
/*
 * Indicate reserved TDBGC counters.
 * reserved_tdbgc_x controls TDBGCx.
 * By default TDBGC0-5 are reserved.
 * 0    --   Unreserve the TDBGC counter.
 * 1    --   Reserve the TDBGC counter.
 */
#define spn_RESERVED_TDBGC  "reserved_tdbgc"
/*
 * Enable Dither for TimeSync PLL.
 * 0    --   Disable.
 * 1    --   Enable.
 */
#define spn_TS_PLL_DITHER_ENABLE  "ts_pll_dither_enable"
/*
 * Enable Holdover for BroadSync PLL.
 * 0    --   Disable.
 * 1    --   Enable.
 */
...
{ \
  ...
  "defip_cam_tm", \
  "defip_wide_key_for_ipmc_disable", \
  ...
  "flowtracker_export_ipfix_enable", \
  ...
  "fp_qual_ipv6_ipv4_overlay_enable", \
  "fpem_mem_entries", \
  "front_panel_esm", \
  "fschan_enable", \
  ...
  "vrrp_scaling_tcam", \
  "vsi_payload_size", \
  "vxlan_tunnel_init_ext_fields_update", \
